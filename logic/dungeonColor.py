...
class DungeonColor:
    def __init__(self, options, world_setup, r):
        entrance = Location(9)
        room2 = Location(9).connect(entrance, r.attack_hookshot_powder)
        room2.add(DungeonChest(0x314))  # key
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
            Location(9).add(OwlStatue(0x308), OwlStatue(0x30F)).connect(room2, STONE_BEAK9)
        room2_weapon = Location(9).connect(room2, r.attack_hookshot)
        room2_weapon.add(DungeonChest(0x311))  # stone beak
        room2_lights = Location(9).connect(room2, OR(r.attack_hookshot, SHIELD))
